<template>
    <header
        class="sticky top-3 z-50 flex items-center justify-between px-6 py-3 md:py-4 shadow max-w-5xl rounded-full mx-auto w-full bg-white transition-all duration-300">
        <a href="https://prebuiltui.com">
            <img
                src="https://raw.githubusercontent.com/prebuiltui/prebuiltui/main/assets/dummyLogo/prebuiltuiDummyLogo.svg" />
        </a>
        <nav id="menu"
            class="max-md:absolute max-md:top-0 max-md:left-0 max-md:overflow-hidden items-center justify-center max-md:h-full max-md:w-0 transition-[width] bg-white/50 backdrop-blur flex-col md:flex-row flex gap-8 text-gray-900 text-md font-normal">

            <router-link v-for="(item, index) in menuItems" :key="index" :to="{ name: item.pathName }"
...
const menuItems = [
    ...
    { title: "Donation", pathName: "Donation" },
];
</script>